...
test("Studio persistence controls load through the browser dev server", async (t) => {
  ...
  assert.match(save, /serializeStudioScene/);
  assert.match(save, /createStudioSceneFilename/);
  assert.match(save, /application\/json/);
  ...
  assert.match(png, /toDataURL\("image\/png"\)/);
  assert.match(png, /createStudioPngFilename/);
});
...
async function waitForServer(server) {
  for (let attempt = 0; attempt < 80; attempt += 1) {
    if (server.process.exitCode !== null) {
      throw new Error(`Studio Vite exited before persistence smoke started.\n${server.getLogs()}`);
    }
